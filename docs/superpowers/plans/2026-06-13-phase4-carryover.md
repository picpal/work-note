...
- [x] **Step 1: 단위 테스트 작성 (AuthServiceTest 보강)**
...
- [x] **Step 3: AuthService.changePassword 구현**
...
- [x] **Step 5: 통합 테스트 작성 (ChangePasswordApiTest 신규)**
...
- [x] **Step 7: ChangePasswordRequest DTO 생성**
...
- [x] **Step 8: AuthController 엔드포인트 추가**
...
- [x] **Step 3: passwordValidation 구현**
...
- [x] **Step 5: AuthApi.changePassword fetch stub 테스트 작성**
...
- [x] **Step 7: AuthApi.changePassword 추가**
...
- [x] **Step 9: ProfileModal.changePw 실 API 배선**
...
- [x] **Step 10: KNOWN_ACTS 에 auth.password.change 추가**
...
- [x] **Step 11: 전체 프런트 테스트 + 타입체크 통과 확인**
...
- [x] **Step 5: useVaultSync write-through + clear-on-success 배선**
...
- [x] **Step 5: useVault 에 loadError 상태 추가**
...
- [x] **Step 6: ConnectionLost 컴포넌트 생성**
...
- [x] **Step 7: App 에서 loadError 차단 분기**
...
- [x] **백엔드 전체 테스트**: `cd backend && ./gradlew test` — 259 green
- [x] **프런트 전체 테스트 + 빌드**: `cd frontend && pnpm test --run && pnpm build` — 144 green, dist 생성
- [x] **bootJar**: `cd backend && ./gradlew bootJar` (frontend dist 포함)
- [x] **server 모드 jar 스모크**: 로그인 → 비번 변경(204, 같은 세션 me 200) → 옛 비번 로그인 실패 → 새 비번 로그인 성공 → 잘못된 현재 비번(422·세션 유지) → 감사 `auth.password.change` 1건
- [x] **local 모드 스모크**: 노트 앱 기존 동작 무변경(ProfileModal 비번 변경 시 "로컬 모드" 안내), 차단 화면 미발생
- [x] 최종 통합 리뷰 subagent: D1–D11 준수 + 교차 정합(세션 salt 갱신/401 제외 판정/write-through 생명주기) 대조
