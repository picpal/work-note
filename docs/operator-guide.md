...
#    데이터 디렉토리가 700이므로 앱 계정이나 root로 실행하세요. 출력은 서버 로컬 시간입니다.
sudo -u worknote stat -c '%y' /data/worknote/break-glass.processing   # 예: 2026-08-27 13:31:02.000000000 +0900
...
- **왜 이 비교가 성립하나**: 센티넬은 소비될 때 지워지므로 지금 남아 있는 파일은 **이번 시도를 위해 새로 만든 것**입니다. 따라서 과거 복구의 흔적은 반드시 mtime보다 앞에 오고, 이번 시도의 흔적은 반드시 뒤에 옵니다. 두 시각 모두 서버 로컬 시간으로 기록되므로 `stat`의 출력과 그대로 비교하면 됩니다.
- **둘이 엇갈리거나 판단이 서지 않으면**(예: 파일을 `cp -p`·`touch -t`로 만들어 mtime이 과거이거나, 그 사이 서버 시계를 조정했다면) **적용되지 않은 것으로 보고 재시도하세요.** 앱이 뜨지 못한 동안에는 아무도 로그인해 2FA를 재등록하거나 비밀번호를 바꿀 수 없었으므로, 다시 적용해도 **되돌아갈 상태가 없습니다.** 앱이 스스로 재시도하지 않는 이유는 앱이 이 사실을 알 수 없기 때문입니다 — 운영자는 알 수 있습니다.

> **DB를 다른 계정으로 열지 마세요.** root로 열면 저널/보조 파일이 root 소유로 생겨 앱이 못 쓰게 됩니다. `sqlite3`가 없는 폐쇄망이라면 **서비스를 중지한 상태에서** DB를 조회 가능한 장비로 복사해 확인하세요. 이 앱은 WAL을 켜지 않지만(`journal_mode=delete`), 복사 시점에 `worknote.db-journal`(설정을 바꿨다면 `-wal`·`-shm`)이 옆에 남아 있으면 **반드시 함께 복사**하세요 — 본체만 복사하면 마지막 커밋이 빠져 "흔적 없음"으로 오판할 수 있습니다.
...
>
> 검증·읽기·선점(`.processing` rename)은 **한 번 연 디렉토리 핸들** 위에서 수행합니다 — 셋을 각각 경로로 다시 해석하면 그 사이에 파일이나 디렉토리가 갈아끼워져 "검사한 것과 다른 파일을 실행"할 수 있기 때문입니다. 이 강한 보장은 **리눅스 배포에서 적용**됩니다(JDK가 `SecureDirectoryStream`을 제공하는 플랫폼). macOS 등 제공하지 않는 플랫폼에서는 **경로 기반으로 동작하며, 위 조건 검사는 완전히 동일하고 그 시간차 창만 넓습니다.** 운영 배포가 리눅스라면 이 차이는 해당되지 않습니다.
>
