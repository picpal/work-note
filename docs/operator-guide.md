...
| **조상 경로(부모의 위 → 루트)가 그룹·타인에게 쓰기 가능하지 않음** | 부모가 `700`이어도 그 위에 쓸 수 있는 사람은 **데이터 디렉토리를 통째로 자기 것으로 바꿔치기**할 수 있습니다(디렉토리 엔트리 rename). 자식의 `700`은 그걸 막지 못하므로, 부모만 봐서는 아래 "왜 백도어가 아닌가"의 근거가 성립하지 않습니다 |
| 4KB 이하 | 센티넬은 두 줄입니다 |

조상 검사에서 보는 것은 **쓰기 비트뿐**입니다 — `/`, `/var`, `/var/lib`, `/srv`가 `755`인 것은 정상이고 문제가 되지 않습니다. 다만 **`/tmp`처럼 sticky가 붙은 디렉토리(`1777`)는 예외로 허용**합니다(sticky 디렉토리에서는 남이 소유한 엔트리를 rename·삭제할 수 없으므로 바꿔치기가 성립하지 않습니다). 거부에 걸리면 메시지가 문제의 경로와 실행할 `chmod`를 지목하므로, 그대로 조치한 뒤 재기동하세요. 조상 검사는 **데이터 디렉토리를 심링크로 붙인 배치**에서 링크가 놓인 디렉토리와 링크 타깃의 조상을 모두 봅니다.
...
서비스가 뜨지 않는 상태이므로 화면으로는 확인할 수 없습니다. **DB를 직접 열어 판단하세요** — 감사 행은 계정 수술과 **같은 트랜잭션**에 들어가므로, 그 행이 있으면 적용된 것이고 없으면 적용되지 않은 것입니다.

```bash
# 반드시 앱 실행 계정으로, 읽기 전용으로 여세요 (root로 열면 -wal/-shm 파일이 root 소유로 생겨 앱이 못 씁니다)
...
# 계정 상태도 함께 (2FA 등록 여부 / 계정 상태·유예 시작 시각)
sudo -u worknote sqlite3 -readonly /data/worknote/worknote.db \
...
- **감사 행이 있다** = 복구가 적용됐습니다(2FA 해제·유예 재시작·지정했다면 비밀번호까지). `.processing` 파일을 지우고 기동한 뒤 로그인하세요.
- **감사 행이 없다** = 아무것도 적용되지 않았습니다(트랜잭션이 통째로 롤백됩니다). `.processing` 파일을 지우고 센티넬을 다시 만들어 기동하세요.
- `sqlite3`가 설치돼 있지 않은 폐쇄망이라면 DB 파일을 조회 가능한 장비로 **복사해서** 확인하세요(원본을 다른 계정으로 열지 마세요).
...
>
> 이 논거는 **주장이 아니라 검사**입니다(위 조건 표). 부모의 `700`만으로는 부족해서 **조상 경로까지** 봅니다 — 그 위 어딘가가 그룹·타인 쓰기 가능하면 데이터 디렉토리를 통째로 바꿔치기해 "700 앱 소유 디렉토리"인 척 만들 수 있고, 그러면 DB를 읽지도 못하던 사람이 관리자 비밀번호를 지정하게 되기 때문입니다. 전제를 확인할 수 없으면 실행하지 않고 **기동을 세웁니다.**
>
...
| 기동 실패: "브레이크글래스 복구: ..." | 센티넬 파일 내용 문제(사번 없음·비밀번호 10자 미만·알 수 없는 키 등) 또는 출처 검증 실패(파일·부모·**조상 경로**의 소유자·권한). 메시지가 원인과 실행할 `chmod`를 지목합니다. 파일은 남아 있으니 고쳐서 재기동하세요 |
