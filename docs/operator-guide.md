...
아무도 로그인할 수 없으므로 아무도 초기화해 줄 수 없습니다. 이때를 위한 경로가 **센티넬 파일**입니다. 서버 호스트에 접근할 수 있는 사람만 쓸 수 있습니다.
...
- `emp`는 **필수**, `password`는 **선택**(10~128자)입니다. `password` 줄을 지우면 2FA만 풀고 비밀번호는 그대로 둡니다(비밀번호는 기억하는데 기기만 잃은 경우).
...
| 상위 디렉토리 소유자 = 앱 실행 계정, **`700`** | 열려 있었다면 누구나 이 자리에 파일을 놓을 수 있었다는 뜻입니다 |
| 4KB 이하 | 센티넬은 두 줄입니다 |
...
**중단된 복구**: 처리 직전에 `break-glass.processing`으로 이름이 바뀌고, 커밋 후 삭제됩니다. 기동 시 이 파일이 남아 있으면 이전 시도가 중간에 끊긴 것이므로 **기동이 실패합니다** — 적용됐는지 아닌지 앱이 알 수 없기 때문입니다. 계정 상태(로그인·2FA)를 확인하고 파일을 지운 뒤 다시 띄우세요.
...
**문제가 있으면 기동이 실패하고 파일은 남습니다** (고쳐서 다시 띄우세요): 사번이 없음 · 비밀번호가 10자 미만 · `password=`가 빈 값 · 알 수 없는 키 · 파일을 읽을 수 없음. **성공 후 파일 삭제에 실패해도 기동이 실패합니다** — 지워지지 않은 센티넬은 재기동할 때마다 되살아나는 뒷문이라, 그 상태로 서비스하느니 멈추는 편이 낫습니다.
...
**센티넬 경로를 바꾸는 설정은 일부러 두지 않았습니다.** 위치는 항상 `<DB 부모>/break-glass` 한 곳입니다 — 경로를 설정으로 열면 공용 디렉토리를 가리키는 오설정 한 번으로 위 전제가 통째로 깨지고, 그것을 막아 주는 것은 아무것도 없기 때문입니다.
...
| **아무도 로그인할 수 없음** (유일 관리자가 2FA 기기 분실 + SMTP 미설정) | 서버 호스트에서 센티넬 파일로 복구하세요 — [최후 복구](#아무도-로그인할-수-없을-때--최후-복구break-glass) |
| 기동 실패: "브레이크글래스 복구: ..." | 센티넬 파일 내용 문제(사번 없음·비밀번호 10자 미만·알 수 없는 키 등). 메시지가 원인을 지목합니다. 파일은 남아 있으니 고쳐서 재기동하세요 |
