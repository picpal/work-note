...
| `WORKNOTE_ATTACHMENT_REAP_GRACE_HOURS` | `24` | 행 없는 첨부 파일(고아) 회수 유예 시간. 이보다 최근에 수정된 파일은 건드리지 않음. 0 이하 = 회수 끔 — [고아 첨부 파일 자동 회수](#고아-첨부-파일-자동-회수) |
...
첨부는 **파일을 먼저 쓰고 DB 행을 나중에 커밋**합니다. 쓰기 실패와 트랜잭션 롤백은 그 자리에서 정리되지만, **그 사이에 프로세스가 강제 종료되면**(kill -9, 전원 차단, OOM) DB에 행이 없는 파일이 디스크에 남습니다. 노트 삭제 시 파일 삭제가 실패한 경우도 같은 결과가 됩니다. 이런 파일은 DB 기준으로 도는 purge가 영영 회수하지 못하므로, 디스크에서 출발하는 회수기가 주기적으로 정리합니다.

- 스케줄: 기동 60초 후 첫 실행, 이후 24시간 간격 (휴지통 purge와 동일한 배선)
- `WORKNOTE_ATTACHMENT_REAP_GRACE_HOURS` (기본 `24`): **이 시간보다 최근에 수정된 파일은 건드리지 않습니다.** 커밋 직전의 업로드를 지우지 않기 위한 유예이며, 이것이 유일한 경합 방어입니다. 짧게 줄이지 마세요. `0` 이하로 두면 회수기가 꺼집니다
- 회수 대상은 앱이 만든 샤딩 경로(`xx/yy/<32자리 16진수>`)에 정확히 맞는 파일**뿐**입니다. `WORKNOTE_UPLOAD_DIR`에 다른 파일이 섞여 있어도 그것들은 대상이 아닙니다 — [데이터 파일 권한](#데이터-파일-권한)의 "앱이 만든 것만 앱이 바꾼다"와 같은 원칙입니다
- **DB 행은 어떤 경우에도 삭제하지 않습니다.** 파일이 사라진 행이 발견되면 지우지 않고 개수만 WARN으로 남깁니다 — 이건 실제 데이터 유실 신호이므로 백업을 확인하세요
...
| 첨부 디렉토리 용량이 DB 기록보다 계속 큼 | 강제 종료로 남은 고아 파일. 기동 60초 후 회수기가 돌며 `WORKNOTE_ATTACHMENT_REAP_GRACE_HOURS`(기본 24) 경과분만 지웁니다 — 최대 유예+24시간 뒤 정리됩니다 ([고아 첨부 파일 자동 회수](#고아-첨부-파일-자동-회수)) |
