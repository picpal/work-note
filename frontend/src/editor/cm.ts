// cm.ts — CodeMirror 6 "Live Preview" markdown editor (Obsidian-style).
...
// is outside them. Ported from cm.js (prototype) — window.* globals replaced with
...
const KEYWORDS: Record<string, { type: string; words: string[] }> = {
...
function currentFenceLang(state: EditorState, pos: number) {
...
function inCodeBlock(state: EditorState, pos: number) {
...
function tabInCode(view: EditorView) {
...
function shiftTabInCode(view: EditorView) {
  if (!inCodeBlock(view.state, view.state.selection.main.head)) return false;
  return indentLess(view);
}
function fenceCompletion(ctx: import("@codemirror/autocomplete").CompletionContext) {
...
class RenderWidget extends WidgetType {
  ...
  source: string;
  pos: number;
  constructor(kind: string, source: string, pos: number) { super(); this.kind = kind; this.source = source; this.pos = pos; }
  eq(o: RenderWidget) { return o.kind === this.kind && o.source === this.source; }
  toDOM(view: EditorView) {
    ...
    if (this.kind === "mermaid") {
      ...
      wrap.appendChild(m);
      requestAnimationFrame(() => { enhanceMermaid(wrap); });
    } else {
      const inner = document.createElement("div");
      inner.className = "md";
      inner.innerHTML = renderMarkdown(this.source);
...
function buildDecorations(state: EditorState) {
  ...
  const lineActive = (pos: number) => { if (!focused) return false; const n = state.doc.lineAt(pos).number; return n >= aFrom && n <= aTo; };
  const selTouches = (from: number, to: number) => focused && sel.from <= to && sel.to >= from;
...
const decoField = StateField.define<import("@codemirror/view").DecorationSet>({
...
export function prefix(view: EditorView, pfx: string): void {
  const line = view.state.doc.lineAt(view.state.selection.main.from);
  view.dispatch({ changes: { from: line.from, insert: pfx }, selection: { anchor: view.state.selection.main.from + pfx.length } });
  view.focus();
}

// Heading toggle: replaces any existing leading "#{1,6} " marker; same level → removes it.
export function heading(view: EditorView, level: number): void {
  const state = view.state;
  const line = state.doc.lineAt(state.selection.main.from);
  const m = line.text.match(/^(#{1,6})\s+/);
  const want = "#".repeat(level) + " ";
  let insert: string, removedLen: number;
  if (m) {
...
export function block(view: EditorView, text: string): void {
  const line = view.state.doc.lineAt(view.state.selection.main.from);
